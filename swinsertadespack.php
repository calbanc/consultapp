...
if($conexion){
    ...
    &&isset($_GET["RUT_CHOFER"])){
        ...
        if($registross=sqlsrv_fetch_array($resultadoantes)){
            ...
            if($TIP_MOV=='T'){
                $consulta="INSERT INTO ANDROID_DESPACK(COD_EMP,COD_TEM,PLANILLA,COD_PACK,COD_FRI,LOTE,ZON,CANTIDAD,CHOFER,PATENTE,RUT_CHOFER,TIP_MOV,ZON_DEST,COD_FRIO_DEST)
                VALUES('{$COD_EMP}','{$COD_TEM}','{$PLANILLA}','{$COD_PACK}','{$COD_FRI}','{$LOTE}','{$ZON}','{$CANTIDAD}','{$CHOFER}','{$PATENTE}','{$RUT_CHOFER}','{$TIP_MOV}','{$ZON_DEST}','{$COD_FRIO_DEST}')";
          
            }else{
                $consulta="INSERT INTO ANDROID_DESPACK(COD_EMP,COD_TEM,PLANILLA,COD_PACK,COD_FRI,LOTE,ZON,CANTIDAD,CHOFER,PATENTE,TIP_REC,COD_REC,RUT_CHOFER,TIP_MOV)
                VALUES('{$COD_EMP}','{$COD_TEM}','{$PLANILLA}','{$COD_PACK}','{$COD_FRI}','{$LOTE}','{$ZON}','{$CANTIDAD}','{$CHOFER}','{$PATENTE}','C','{$COD_REC}','{$RUT_CHOFER}','{$TIP_MOV}')";
