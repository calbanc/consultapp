...
if($conexion){
	...
	if(isset($_GET["Correlativo"])&&isset($_GET["cantidad"])&&isset($_GET["IdFamilia"])&&isset($_GET["IdLabor"])&&isset($_GET["Etapa"])){
		...
		$consulta="UPDATE ANDROID_AsistenciaRapida SET Cantidad='{$cantidad}', IdFamilia='{$IdFamilia}' ,IdActividad='{$IdLabor}' ,Etapa='{$Etapa }' WHERE Llave='{$Correlativo}' and SW_APROBADO ='0'" ;
