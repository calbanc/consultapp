...
if($conexion){
    ...
    &&isset($_GET["COD_CUA"])&&isset($_GET["PLANILLA"])){
        ...
        $PLANILLA=$_GET["PLANILLA"];
            ...
            $consultaplanillaantes="SELECT DISTINCT PLANILLA FROM ANDROID_COSECHA_HUERTO_DETALLE WHERE COD_EMP='{$COD_EMP}' AND COD_TEM='{$COD_TEM}' AND IDANDROID='{$PLANILLA}' ";
            
            $queryantes=sqlsrv_query($conexion,$consultaplanillaantes);
            if($registros=sqlsrv_fetch_array($queryantes)){
