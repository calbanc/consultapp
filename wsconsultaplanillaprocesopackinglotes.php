...
if($conexion){
	...
	if(isset($_GET["COD_EMP"])&&isset($_GET["COD_TEM"])&&isset($_GET["PLANILLA"])&&isset($_GET["ZON"])){
        ...
        $consulta1="SELECT LOTE, COD_ESP, COD_VAR, COD_ENV, COD_CAL, CANTIDAD FROM PROCEPACK 
        WHERE PLANILLA = '{$PLANILLA}' AND  TIPOFRU = 'E' AND  ZON = '{$ZON}' AND  COD_EMP =  '{$COD_EMP}'  AND  COD_TEM = '{$COD_TEM}'";
