...
if($conexion){
    ...
    &&isset($_GET["VERSION"])&&isset($_GET["LLAVE"])&&isset($_GET["TIPO_CARGA"])){
        ...
        if($registro=sqlsrv_fetch_array($resultadoantes)){
            ...
            if($llave==$LLAVE){
                ...
                echo json_encode($json);
                        ...
                        $consulta="INSERT INTO ANDROID_CONSUMO_COMBUSTIBLE (COD_EMP, COD_TEM, PLANILLA, IDUSUARIO, FECHA, IDEMPRESA, IDTRABAJADOR, COD_MAQ, SUBITEM, CANTIDAD, ODOMETRO, HOROMETRO,LLAVE,VERSION,ZON,COD_BOD,TIPO_CARGA)        
                VALUES ('{$COD_EMP}','{$COD_TEM}','{$PLANILLA}','{$IDUSUARIO}','{$FECHA}','{$IDEMPRESA}','{$IDTRABAJADOR}','{$COD_MAQ}','{$SUBITEM}','{$CANTIDAD}','{$ODOMETRO}','{$HOROMETRO}','{$LLAVE}','{$VERSION}','{$idzona}','{$cod_bod}','{$TIPO_CARGA}')";
