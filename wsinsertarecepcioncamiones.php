...
if($conexion){
    ...
    &&isset($_GET["Observacion"])){
        ...
        if($registross=sqlsrv_fetch_array($resultadoantes)){
            ...
            $consulta="INSERT INTO ANDROID_RECEPCION_CAMIONES(COD_EMP,COD_TEM,Fecha_ingreso,RutConductor,NombreConductor,EmpresaTransporte,Patente,Guia_ingreso,Carga,Motivo,Telefono,Observacion,Contenedor,UsuarioSis_ingreso,Sw_Despachado,ZON,COD_TRP,TIPO_CAMION,COD_TIP_CON)
                    VALUES('{$COD_EMP}','{$COD_TEM}','{$Fecha_ingreso}','{$RutConductor}','{$NombreConductor}','{$EmpresaTransporte}','{$Patente}','{$Guia_ingreso}','{$Carga}','{$Motivo}','{$Telefono}','{$Observacion}','{$Contenedor}','{$usuario}','0','{$idzona}','{$cod_trp}','{$TIPO_CAMION}','{$COD_TIP_CON}')";
