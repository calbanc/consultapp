...
if($conexion){
    ...
    &&isset($_GET["COD_CUA"])){
		...
		$consulta="INSERT INTO PROPACKMIX_MASIVO (COD_TEM, COD_EMP,PROCESO,FECHA_PAC,COD_PACK,COD_PRO,COD_FRI,COD_ESP,COD_VAR,ALTURA,TIPO_LOT,LOTE,COD_ENV,COD_ETI,COD_EMB,PLU,CAJAS,COD_CAT,COD_CAL,ZON,COD_ENVOP,COD_VAR_ETI,COD_BP,COD_MER4,NRO_MIX,COD_PRE,COD_CUA)
        VALUES ('{$COD_TEM}','{$COD_EMP}',NULL,'{$FECHA_PAC}','{$COD_PACK}','{$COD_PRO}','{$COD_FRI}','{$COD_ESP}','{$COD_VAR}','{$ALTURA}','{$TIPO_LOT}','{$LOTE}','{$COD_ENV}','{$COD_ETI}','{$COD_EMB}','{$PLU}','{$CAJAS}', '{$COD_CAT}', '{$COD_CAL}','{$ZON}', '{$COD_ENVOP}', '{$COD_VAR_ETI}', '{$COD_BP}', '{$COD_MER4}', '{$NRO_MIX}', '{$COD_PRE}','{$COD_CUA}')";
