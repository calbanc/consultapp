...
if($conexion){
	...
	if(isset($_GET["fechauno"])&&isset($_GET["fechados"])){
		...
		$consulta="select count(causa)cantidad,fecha from covid where fecha BETWEEN '{$fechauno}' and '{$fechados}' and causa='SIN ALCOHOL' and idempresa in ('9','14')  group by fecha ";
