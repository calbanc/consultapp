...
if($conexion){
        ...
        &&isset($_GET["Fecha_ingreso"])
            ...
            if(!empty($IDTRABAJADOR)){
                ...
                ISNULL(CONVERT(varchar,Fecha_salida,120),'') as 'Fecha_salida',Id,Patente,TIPO,RutConductor,ISNULL(Telefono,'') as 'Telefono',ISNULL(Contenedor,'') AS 'Contenedor'
                FROM ANDROID_RECEPCION_CAMIONES
                WHERE COD_EMP='{$COD_EMP}' AND COD_TEM='{$COD_TEM}'  AND CONVERT(date,Fecha_ingreso,103)='{$Fecha_ingreso}' AND UsuarioSis_ingreso='{$usuario}' ".$WHERE." and fecha_salida is null and ZON='{$ZON}' order by Fecha_salida  " ;
