...
if($conexion){
    ...
    &&isset($_GET["TEMPERATURA"])){
        ...
        if(!empty($nombretrabajador)){
            ...
            if($COD_EMP=='ARAP' || $COD_EMP=='14'){

                if($TIPO=="ACTUALIZACION"){
                    $consulta="UPDATE ANDROID_RECEPCION_CAMIONES SET ZON='{$idzona}',COD_EMP='{$COD_EMP}',COD_TEM='{$COD_TEM}',Fecha_ingreso='{$HORAINGRESO}',PRESENTACI='{$PRESENTACI}',COINCIDECI='{$COINCIDECI}',
                    ...
                    VALUES('{$COD_EMP}','{$COD_TEM}','{$HORAINGRESO}','{$RUT}','{$NOMBRE}','{$PROCEDENCIA}','{$TELEFONO}','{$PRESENTACI}','{$COINCIDECI}','{$PATENTE}','{$ENTREGATRIP}','{$DESTINO}','{$usuario}','0','{$idzona}','{$MOTIVO}','{$AUTORIZADO}','{$TARJETA}','{$OBSERVACION}','{$TEMPERATURA}','VISITA','{$NROLICENCIA}','{$SCRTVIGENTE}','{$ACOMPANADO}','{$FECHAINICIOSCTR}','{$FECHATERMINOSCTR}','{$CUENTACONSALUD}','{$OBSERVACIONSALUD}')";
