...
if(isset($_GET["usuario"])&&isset($_GET["clave"])){
    ...
    if($conexion){
            $json=array();
            if(isset($_GET["COD_EMP"])&&isset($_GET["COD_TEM"])&&isset($_GET["MES"])&&isset($_GET["ANIO"])&&isset($_GET["IdRendidor"]) ){
                
                $COD_EMP=$_GET["COD_EMP"];
                $COD_TEM=$_GET["COD_TEM"];
                $MES=$_GET["MES"];
                $ANIO=$_GET["ANIO"];
                $IdRendidor=$_GET["IdRendidor"];
                $sqlwhere="";

                if($IdRendidor<>"TODOS"){
                    $sqlwhere="AND R.IDTRABAJADOR='{$IdRendidor}'";
                }

                $consulta="SELECT[Empresa]= E.NOM_EMP,[Temporada]=R.COD_TEM,[Id Rendidior]=R.IDTRABAJADOR, [Rendidor]=T.NOMBRE+' '+T.APELLIDOPATERNO+' '+T.APELLIDOMATERNO,
                [Id Gasto]=R.IDMANGASTO, [Item]=G.DESCRIPCION, [Año]=YEAR(R.FECHA), [Mes]=MONTH(R.FECHA), [Fecha]=CONVERT(VARCHAR,R.FECHA,103),       
                ...
                ,TOTAL_US =R.TOTAL/D.DOLAR	, R.CODIGOCLIENTE, R.CANTIDAD, R.HOROMETRO, R.ODOMETRO, G.SW_COMBUSTIBLE, R.APROBACION, R.IMPORTADO,JD.UsuarioSis,R.ENTREGADINERO,R.COD_EMP
                ...
                WHERE R.COD_EMP='{$COD_EMP}' AND R.COD_TEM='{$COD_TEM}'  and JD.UsuarioSis='{$usuario}' and R.APROBACION='0' and YEAR(R.FECHA)='{$ANIO}' and MONTH(R.FECHA)='{$MES}' ".$sqlwhere;
